...
const REPAYMENT_STYLES = ['Sculpted (target DSCR)','Level debt service','Equal principal','Bullet','IO then amortize','Deferred P&I then sculpted','Custom schedule'];
  ...
  return <div>
    <Section title="Optimizer Mode">
      ...
      <p className="text-xs text-stone-500 mt-3">
        {mode==='single'
          ? 'Size one instrument against your binding constraints (binary search).'
          : 'Size senior, then sub, sequentially against residual CFADS. Equity (or chosen plug) absorbs the funding gap.'}
      </p>
    </Section>
...
export default function App(){
  ...
  const loadScenario = async (key)=>{
    try { const res = await window.storage.get(key); if(res?.value) setModel(JSON.parse(res.value)); } catch(e){ console.error(e); }
  };
